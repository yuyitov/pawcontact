 ...
 *
 * Se emparejan POR ORDEN, no por título: una pregunta que declara `name` hace
 * que Tally reporte el NOMBRE como título — el mecanismo exacto que tiró 11 de
 * 17 campos del intake de PawContact el 2026-07-26. En ESTE formulario TODAS
 * las preguntas declaran `name`, así que el título del webhook es casi siempre
 * el nombre estable; por eso el `title_editor` (lo que el cliente LEE) se
 * guarda aparte cuando difiere.
 *
...
const salida = {
  _comment:
    'Inventario de los DOS formularios Tally VIVOS de PawContact (EN y ES). Solo ids, tipos, ' +
    ...
    'ese archivo es la ESPECIFICACION, y el formulario vivo puede haberse quedado atras (paso ' +
    'el 2026-07-26). Medir contra el yaml y creerle es el error que trajo el plan de cero ' +
    'regresiones.',
  snapshot_date: hoy,
  forms: [
